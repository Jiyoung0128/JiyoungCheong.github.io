...
Drug abuse is a huge social problem in the U.S. We tried to build prediction model to drug use frequency. We used National Survey on Drug Use and Health (NSDUH) data provided by Substance Abuse and Mental Health Service Administration (SAMHSA). The target variable we used was 'the number of days one used marijuana in one year.' We first trained a random forest regressor, whose cross-validated $R^2$ ranged from -6.8 to -2.7  and applied Yeo Johnson and Bayesian optimization for better performance which showed cross validated mean $R^2$: 0.23. Also we further discussed about ethical issues involved in using data such as race, economic status, and educational level to predict drug use frequency.   
